...
const initialCoachApplications = [
    {
        id: 1,
        firstName: "Sarah",
        lastName: "Johnson",
...
export default function AdminApprovalPage({ onBack }) {
    ...
    const [coachApplications, setCoachApplications] = useState(initialCoachApplications)
    ...
    const handleCoachExpand = (coachId) => {
    ...
    const executeAction = () => {
        ...
        if (category === "coach") {
            setCoachApplications((prev) => prev.map((coach) => (coach.id === item.id ? { ...coach, status: type } : coach)))
        } else {
        ...
        // Show success alert
        const actionText = type === "approved" ? "approved" : "denied"
        const itemName = category === "coach" ? `${item.firstName} ${item.lastName}` : item.workshopTitle
        alert(
            `${category === "coach" ? "Coach application" : "Workshop request"} for "${itemName}" has been ${actionText} successfully!`,
        )
    }
                ...
                {coachApplications.map((coach) => (
                    ...
                    >
                        ...
                        <Box
                            ...
                            onClick={() => handleCoachExpand(coach.id)}
                        >
                            <Stack direction="row" alignItems="center" spacing={2}>
                                ...
                                <Chip
                                    ...
                                    sx={{
                                        bgcolor: getStatusColor(coach.status),
                                        color: "white",
                                        fontWeight: "600",
                                    }}
                                />
                                <IconButton>{expandedCoach === coach.id ? <ExpandLess /> : <ExpandMore />}</IconButton>
                            </Stack>
                        </Box>

                        {/* Expanded Coach Details */}
                        <Collapse in={expandedCoach === coach.id}>
                            <Divider />
                                            ...
                                            <Grid item xs={12} md={6}>
                                                <Box>
                                                    ...
                                                    <Stack direction="row" alignItems="center" spacing={1}>
                                                        ...
                                                        <Typography variant="body2">{formatDate(coach.dateOfBirth)}</Typography>
                                                    </Stack>
                                                </Box>
                                            </Grid>
                                        </Grid>
                                        ...
                                        <Grid item xs={12}>
                                            <Stack direction="row" spacing={2} justifyContent="center" sx={{ mt: 3 }}>
                                                <Button
                                                    ...
                                                    onClick={(e) => {
                                                        e.stopPropagation()
                                                        handleConfirmAction("approved", coach, "coach")
                                                    }}
                                                    sx={{
                                                        bgcolor: "#4CAF50",
                                                ...
                                                <Button
                                                    ...
                                                    onClick={(e) => {
                                                        e.stopPropagation()
                                                        handleConfirmAction("denied", coach, "coach")
                                                    }}
                                                    sx={{
                                                        bgcolor: "#F44336",
    ...
    const WorkshopRequestsTab = () => (
        <Box sx={{ p: 4 }}>
            <Stack spacing={3}>
                {workshopRequests.map((workshop) => (
                    <Card
                        key={workshop.id}
                        sx={{
                            border: "2px solid #F0F0F0",
    ...
    return (
        <Box sx={{ minHeight: "100vh", bgcolor: "#FFFFFF", p: 4 }}>
            ...
            <div className="justify-between flex items-center">
                <Box sx={{ mb: 4 }}>
                    ...
                    <Typography variant="h6" sx={{ color: "#666666" }}>
                        Review and approve coach applications and workshop requests
                    </Typography>
                </Box>
                <Button
                variant="contained"
            ...
            <Card sx={{ borderRadius: 4, boxShadow: "0 8px 32px rgba(0,0,0,0.12)", overflow: "hidden" }}>
                ...
                <Box sx={{ bgcolor: "#000000" }}>
                    ...
                    >
                        <Tab
                            icon={<Person sx={{ mr: 1 }} />}
                            iconPosition="start"
                            label={`Coach Applications (${coachApplications.filter((c) => c.status === "pending").length})`}
                        />
                        <Tab
                            icon={<Event sx={{ mr: 1 }} />}
                            iconPosition="start"
                ...
                {/* Tab Content */}
                {activeTab === 0 && <CoachApplicationsTab />}
                {activeTab === 1 && <WorkshopRequestsTab />}
            </Card>
            ...
            >
                <DialogTitle sx={{ fontWeight: "700", color: "#000000" }}>
                    Confirm {confirmDialog.type === "approved" ? "Approval" : "Denial"}
                </DialogTitle>
                <DialogContent>
                    <Typography variant="body1" sx={{ color: "#666666" }}>
                        Are you sure you want to {confirmDialog.type === "approved" ? "approve" : "deny"} this{" "}
                        {confirmDialog.category === "coach" ? "coach application" : "workshop request"}?
                    </Typography>
                    {confirmDialog.item && (
                        <Alert severity={confirmDialog.type === "approved" ? "success" : "warning"} sx={{ mt: 2, borderRadius: 2 }}>
                            <Typography variant="body2" sx={{ fontWeight: "600" }}>
                                {confirmDialog.category === "coach"
                                    ? `${confirmDialog.item.firstName} ${confirmDialog.item.lastName}`
                                    : confirmDialog.item.workshopTitle}
                            </Typography>
                            <Typography variant="body2">
                                {confirmDialog.category === "coach" ? "Coach Application" : `by ${confirmDialog.item.coachName}`}
                            </Typography>
                        </Alert>
                    )}
                </DialogContent>
                <DialogActions sx={{ p: 3 }}>
                    ...
                    <Button
                        ...
                        variant="contained"
                        sx={{
                            bgcolor: confirmDialog.type === "approved" ? "#4CAF50" : "#F44336",
                            "&:hover": {
                                bgcolor: confirmDialog.type === "approved" ? "#45A049" : "#E53935",
                            },
                            borderRadius: 2,
                        }}
                    >
                        {confirmDialog.type === "approved" ? "Approve" : "Deny"}
                    </Button>
                </DialogActions>
            </Dialog>
            <MediaPopup />
